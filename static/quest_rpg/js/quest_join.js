(function(){
  ...
  async function apiPost(url, body={}){
    ...
    try{
      ...
      console.error('[Quest] Network error POSTing to', url, networkErr);
      throw new Error('Could not reach the server. Check your connection.');
    }

    // Read the body as text first so we can show the real error even when
    // the server returns an HTML error page (500/403/404) instead of JSON.
    const raw = await res.text();
    let data = null;
    try{ data = raw ? JSON.parse(raw) : {}; }
    catch(parseErr){
      console.error('[Quest] Non-JSON response', res.status, res.statusText, 'from', url, '\n', raw.slice(0, 800));
      throw new Error(`Server returned ${res.status} ${res.statusText} (not JSON). Open the console to see the response.`);
    }

    if(data && data.session){ session = data.session; }
    if(!res.ok || data.ok === false){
      console.error('[Quest] Request failed', res.status, url, data);
